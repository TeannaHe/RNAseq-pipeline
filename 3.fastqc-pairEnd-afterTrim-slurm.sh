...
#SBATCH -o /data120/home/users/jiaying/slurm_log/fastqc-after-trim_%A_%a.out
#SBATCH -e /data120/home/users/jiaying/slurm_log/fastqc-after-trim_%A_%a.err
...
# ===== 检查输入文件是否存在 =====
if [ ! -f "${FASTQ_DIR}/${sample}_R1_trimmed.fastq.gz" ] || [ ! -f "${FASTQ_DIR}/${sample}_R2_trimmed.fastq.gz" ]; then
	echo "ERROR: Trimmed Fastq files not found for ${sample}."
	exit 1
fi

# ===== 跳过已完成的样本 =====
if [ -f "${FASTQ_DIR}/${sample}_R1_trimmed_fastqc.html" ] && [ -f "${FASTQ_DIR}/${sample}_R2_trimmed_fastqc.html" ]; then
...
fastqc \
	...
	$FASTQ_DIR/${sample}_R2_trimmed.fastq.gz
